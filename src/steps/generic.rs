...
pub fn run_mise(ctx: &ExecutionContext) -> Result<()> {
    ...
    ctx.execute(&mise)
        ...
        .arg_if(ctx.config().mise_no_config(), "--no-config")
        .arg_if(ctx.config().mise_no_env(), "--no-env")
        .arg_if(ctx.config().mise_no_hooks(), "--no-hooks")
        .arg_if(ctx.config().mise_locked(), "--locked")
        .arg_if(ctx.config().mise_raw(), "--raw")
