...
mod helpers {
    ...
    pub fn run_simple(ctx: &ExecutionContext, cmd_name: &str, sep: &str, args: &[&str]) -> Result<()> {
...
mod package_managers {
    ...
    pub fn run_flutter_upgrade(ctx: &ExecutionContext) -> Result<()> {
        run_simple(ctx, "flutter", "Flutter", &["upgrade"])
    }

    pub fn run_gem(ctx: &ExecutionContext) -> Result<()> {
        run_simple(ctx, "gem", "RubyGems", &["update"])
    }
    ...
    pub fn run_rubygems(ctx: &ExecutionContext) -> Result<()> {
        ...
        // Check if using rbenv or rvm - they don't need sudo
        let gem_path = gem.clone();
        let gem_path_str = gem_path.as_os_str();
        let directory_writable = gem_path_str
            .to_str()
            .map(|s| s.contains(".rbenv") || s.contains(".rvm"))
            .unwrap_or(false);

        let mut command = if directory_writable {
            ctx.run_type().execute(&gem)
        } else {
    ...
    pub fn run_sheldon(ctx: &ExecutionContext) -> Result<()> {
        run_simple(ctx, "sheldon", "Sheldon", &["lock", "--update"])
    }

    pub fn run_fossil(ctx: &ExecutionContext) -> Result<()> {
        run_simple(ctx, "fossil", "Fossil", &["all", "sync"])
    }
    ...
    pub fn run_micro(ctx: &ExecutionContext) -> Result<()> {
        ...
        ctx.run_type().execute(&micro).arg("update").status_checked()
    }
    ...
    pub fn run_apm(ctx: &ExecutionContext) -> Result<()> {
        run_simple(ctx, "apm", "Atom Package Manager", &["upgrade", "--confirm=false"])
    }
    ...
    pub fn run_uv(ctx: &ExecutionContext) -> Result<()> {
        ...
        if version < Version::new(0, 4, 25) {
            ...
            let output = match output {
                ...
                ExecutorOutput::Dry => {
                    unreachable!("the whole function returns when we run `uv --version` under dry-run")
                }
            };
...
mod language_runtimes {
    ...
    pub fn run_rye(ctx: &ExecutionContext) -> Result<()> {
        run_simple(ctx, "rye", "Rye", &["self", "update"])
    }
...
mod jetbrains {
    ...
    // Define a macro to reduce repetition for JetBrains IDEs
    macro_rules! define_jetbrains_ide_update {
        ($func_name:ident, $binary:expr, $name:expr) => {
            pub fn $func_name(ctx: &ExecutionContext) -> Result<()> {
                run_jetbrains_ide(ctx, require($binary)?, $name)
            }
        };
    }

    // Define all JetBrains IDE update functions using the macro
    define_jetbrains_ide_update!(run_jetbrains_aqua, "aqua", "Aqua");
    define_jetbrains_ide_update!(run_jetbrains_clion, "clion", "CLion");
    define_jetbrains_ide_update!(run_jetbrains_datagrip, "datagrip", "DataGrip");
    define_jetbrains_ide_update!(run_jetbrains_dataspell, "dataspell", "DataSpell");
    define_jetbrains_ide_update!(run_jetbrains_gateway, "gateway", "Gateway");
    define_jetbrains_ide_update!(run_jetbrains_goland, "goland", "Goland");
    define_jetbrains_ide_update!(run_jetbrains_idea, "idea", "IntelliJ IDEA");
    define_jetbrains_ide_update!(run_jetbrains_mps, "mps", "MPS");
    define_jetbrains_ide_update!(run_jetbrains_phpstorm, "phpstorm", "PhpStorm");
    define_jetbrains_ide_update!(run_jetbrains_pycharm, "pycharm", "PyCharm");
    define_jetbrains_ide_update!(run_jetbrains_rider, "rider", "Rider");
    define_jetbrains_ide_update!(run_jetbrains_rubymine, "rubymine", "RubyMine");
    define_jetbrains_ide_update!(run_jetbrains_rustrover, "rustrover", "RustRover");
    define_jetbrains_ide_update!(run_jetbrains_webstorm, "webstorm", "WebStorm");
}
