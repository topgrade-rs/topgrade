...
impl Executor {
    ...
    ///
    /// This converts `Dry` to `Damp` (which prints and executes), while leaving
    /// `Wet` and `Damp` unchanged.
    pub fn always(self) -> Self {
        match self {
            Executor::Dry(c) => Executor::Damp(c.into_command()),
            other => other,
        }
    }
...
mod test {
    ...
    fn test_dry_always_executes() {
        ...
        assert!(matches!(executor, Executor::Damp(_)));
