...
impl Terminal {
    ...
    fn notify_desktop<P: AsRef<str>>(&self, message: P, timeout: Option<Duration>) {
        debug!("{}: {}", t!("Desktop notification"), message.as_ref());
    ...
    fn print_error<P: AsRef<str>, Q: AsRef<str>>(&mut self, key: Q, message: P) {
        ...
        self.term
            .write_fmt(format_args!(
                "{} {}",
                style(format!("{key} {}:", t!("failed"))).red().bold(),
                message
            ))
            .ok();
    }
    ...
    fn should_retry(&mut self, interrupted: bool, step_name: &str) -> eyre::Result<bool> {
        ...
        if self.desktop_notification {
            self.notify_desktop(format!("{step_name} {}", t!("failed")), None);
        }
        ...
        let answer = loop {
            match self.term.read_key() {
                ...
                Err(e) => {
                    error!("{}: {}", t!("Error reading from terminal"), e);
                    break Ok(false);
                }
